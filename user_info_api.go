...
type userIDListResp struct {
	...
	// 当一个成员属于多个部门时 会返回多次
	DeptUser []struct {
		DeptID int64  `json:"department"`
		UserID string `json:"userid"`
	} `json:"dept_user"`
}

func (r userIDListResp) ToUserListResp() userListResp {
	var resp userListResp
	resp.CommonResp = r.CommonResp
	var m = make(map[string][]int64)
	for _, v := range r.DeptUser {
		m[v.UserID] = append(m[v.UserID], v.DeptID)
	}
	for k, v := range m {
		resp.Users = append(resp.Users, &userDetailResp{
			UserID:         k,
			DeptIDs:        v,
			DeptOrder:      make([]uint32, len(v)),
			IsLeaderInDept: make([]int, len(v)),
