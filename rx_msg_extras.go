...
type rxMessageUnknown struct {
	Body []byte
}

func (r rxMessageUnknown) formatInto(writer io.Writer) {
	_, _ = fmt.Fprintf(writer, "unknown message: %s", r.Body)
}
...
func extractMessageExtras(common rxMessageCommon, body []byte) (messageKind, error) {
	...
	case MessageTypeEvent:
		...
		case EventTypeChangeExternalContact:
			...
			default:
				return rxMessageUnknown{Body: body}, nil
			}
		...
		case EventTypeChangeExternalTag:
			...
			case ChangeTypeDeleteTag:
				...
				return &x, nil
			case ChangeTypeShuffleTag:
				var x rxEventShuffleTag
				err := xml.Unmarshal(body, &x)
				if err != nil {
					return nil, err
				}
...
func (r rxEventShuffleTag) formatInto(w io.Writer) {
	_, _ = fmt.Fprintf(
		w,
		"ID: %#v",
		r.ID,
...
var _ EventShuffleTag = (*rxEventShuffleTag)(nil)

func (r *rxEventShuffleTag) GetID() string {
	return r.ID
}
...
var _ EventCustomerServiceExtras = (*rxEventCustomerService)(nil)

func (r rxEventCustomerService) formatInto(writer io.Writer) {
	_, _ = fmt.Fprintf(
		writer,
		"ToUserName: %#v, CreateTime: %#v, MsgType: %#v, Token: %#v, OpenKfId: %#v",
		r.ToUserName,
		r.CreateTime,
		r.MsgType,
		r.Token,
...
func (r rxEventCustomerService) GetToUserName() string {
	return r.ToUserName
}

func (r rxEventCustomerService) GetCreateTime() int64 {
	return r.CreateTime
}

func (r rxEventCustomerService) GetMsgType() string {
	return r.MsgType
}

func (r rxEventCustomerService) GetToken() string {
	return r.Token
}
