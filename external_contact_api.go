...
import (
	...
	"github.com/smart-unicom/wecom/errcodes"
)
...
func (c *App) execExternalContactGet(req externalContactGetReq) (externalContactGetResp, error) {
	...
	// 不存在联系人关系
	if resp.ErrCode == errcodes.ErrCode84061 {
		return externalContactGetResp{}, nil
	}
	if bizErr := resp.TryIntoErr(); bizErr != nil {
...
func (c *App) execExternalContactBatchList(req externalContactBatchListReq) (externalContactBatchListResp, error) {
	...
	// 无用户
	if resp.ErrCode == errcodes.ErrCode84061 {
		return externalContactBatchListResp{}, nil
	}
	if bizErr := resp.TryIntoErr(); bizErr != nil {
...
func (c *App) execExternalContactMarkTag(req externalContactMarkTagReq) (externalContactMarkTagResp, error) {
	...
	if resp.ErrCode == errcodes.ErrCode84061 {
		return externalContactMarkTagResp{}, errors.New("已流失客户 无法添加标签")
	}
	if bizErr := resp.TryIntoErr(); bizErr != nil {
...
func (c *App) execExternalContactGetUserBehaviorData(req externalContactGetUserBehaviorDataReq) (ExternalContactGetUserBehaviorDataRsp, error) {
	var resp ExternalContactGetUserBehaviorDataRsp

	err := c.executeWXApiJSONPost("/cgi-bin/externalcontact/get_user_behavior_data", req, &resp, true)
	if err != nil {
		return ExternalContactGetUserBehaviorDataRsp{}, errors.WithStack(err)
	}

	if bizErr := resp.TryIntoErr(); bizErr != nil {
		return ExternalContactGetUserBehaviorDataRsp{}, bizErr
	}
...
type batchExternalContactListReq struct {
	UserIdList []string `json:"userid_list"`
	Cursor     string   `json:"cursor"`
	Limit      int      `json:"limit"`
}

var _ bodyer = batchExternalContactListReq{}

func (x batchExternalContactListReq) intoBody() ([]byte, error) {
	result, err := json.Marshal(x)
	if err != nil {
		// should never happen unless OOM or similar bad things
		// TODO: error_chain
...
type batchExternalContactListRsp struct {
	CommonResp
	ExternalContactList []ExternalContactInfo `json:"external_contact_list"`
}
...
type externalContactBatchListReq struct {
	UserIdsList []string `json:"userid_list"`
	Cursor      string   `json:"cursor"`
	Limit       int      `json:"limit"`
}
...
type externalContactGetUserBehaviorDataReq struct {
	UserId    []string `json:"userid"`
...
var _ bodyer = externalContactGetUserBehaviorDataReq{}

func (x externalContactGetUserBehaviorDataReq) intoBody() ([]byte, error) {
	result, err := json.Marshal(x)
	if err != nil {
