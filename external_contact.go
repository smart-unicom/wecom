...
func (c *App) AddExternalContactCorpTag(req ExternalContactCorpTagGroup) (ExternalContactCorpTagGroup, error) {
	...
	if reflect.TypeOf(err) == reflect.TypeOf(&ClientError{}) && err.(*ClientError).Code == errcodes.ErrCode40071 {
		return ExternalContactCorpTagGroup{}, errors.New("标签名字已经存在")
	}
...
func (c *App) GetUserBehaviorDataExternalContact(userIds []string, partyIds []int, startTime, endTime int64) ([]ExternalContactGetUserBehaviorData, error) {
	resp, err := c.execExternalContactGetUserBehaviorData(externalContactGetUserBehaviorDataReq{
		UserId:    userIds,
		PartyId:   partyIds,
		StartTime: startTime,
		EndTime:   endTime,
